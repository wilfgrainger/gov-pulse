...
async function loadUseMetrics(
  ...
  buildSnapshot: unknown = null
) {
  vi.resetModules();
  vi.doMock("@/app/generated/metricsSnapshot", () => ({
    BUILD_METRICS_SNAPSHOT: buildSnapshot,
  }));
...
describe("useMetrics", () => {
  afterEach(() => {
    ...
    vi.unstubAllEnvs();
    vi.clearAllMocks();
    vi.doUnmock("@/app/generated/metricsSnapshot");
  });
  ...
  it("normalizes cache state against the source-specific publication window", async () => {
    ...
    expect(
      normalizeCacheState("2026-07-01T00:00:00Z", "expired", monthlyWindow, now)
    ).toBe("fresh");
    expect(
      normalizeCacheState("2026-05-22T00:00:00Z", "fresh", monthlyWindow, now)
    ).toBe("stale");
    expect(
      normalizeCacheState("2026-03-01T00:00:00Z", "fresh", monthlyWindow, now)
    ).toBe("expired");
    expect(
      normalizeCacheState("2026-07-01T00:00:00Z", "missing", monthlyWindow, now)
    ).toBe("missing");
  });
  ...
  it("accepts only live payloads that satisfy the complete fallback shape", async () => {
    ...
    expect(
      acceptsCompleteLivePayload(fallback, {
        economicData: completeLivePayload.economicData,
      })
    ).toBe(false);
    expect(
      acceptsCompleteLivePayload(fallback, {
        ...completeLivePayload,
        metricConfig: {
          ...completeLivePayload.metricConfig,
          inflation: { current: "3.1%" },
        },
      })
    ).toBe(false);
    expect(
  ...
  it("derives a sourced result from the build-time publication snapshot", async () => {
  ...
  it("renders the build snapshot immediately and retains it when browser refresh fails", async () => {
    const buildPayload = {
      ...completeLivePayload,
      economicData: [{ date: "Build", inflation: 3, bankRate: 4, unemployment: 5 }],
    };
    vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new Error("snapshot unavailable")));

    const { useMetrics } = await loadUseMetrics(
      "production",
      snapshotWith(buildPayload)
    );
    const { result } = renderHook(() => useMetrics("sentimentPulse", fallback));

    expect(result.current.source).toBe("snapshot");
    expect(result.current.data.economicData[0].date).toBe("Build");
  ...
  it("replaces build-time evidence with a newer valid same-origin snapshot", async () => {
    const buildPayload = {
      ...completeLivePayload,
      economicData: [{ date: "Build", inflation: 3, bankRate: 4, unemployment: 5 }],
    };
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue({
        ok: true,
        json: async () => snapshotWith(),
      })
    );

    const { useMetrics } = await loadUseMetrics(
      "production",
      snapshotWith(buildPayload, minutesAgo(10))
    );
    const { result } = renderHook(() => useMetrics("sentimentPulse", fallback));
  ...
  it("fails closed when no build or runtime snapshot is available", async () => {
    vi.stubGlobal(
      "fetch",
